...
export const SubscriptionItem = ({ 
    ...
    <View style={{backgroundColor: "#EFEFEF", flexDirection: "row", padding: 10, justifyContent:"space-between",marginTop: 10}}>
        ...
        <View style={{justifyContent: 'center', flexDirection: 'column'}}>
            ...
            <View style={{flex: 1}}><AppText style={{fontSize: 13, fontWeight: String(400), textAlign: 'right'}}> {subscription.type_name} </AppText></View>
            :
            <View style={{flex: 1}}><AppText style={{fontSize: 13, color:'red',  fontWeight: String(400), textAlign: 'right'}}> {subscription.status} </AppText></View>
            }
            <View style={{flex: 1}}><AppText style={{fontSize: 13, color: '#31D457', fontWeight: String(400), textAlign: 'right'}}> {subscription.price_formatted} </AppText></View>
        </View>             
    </View>
    </TouchableOpacity>
  );
